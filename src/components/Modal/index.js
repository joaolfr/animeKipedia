...
const MovieModal = ({
  ...
  let uri =
    'https://ik.imagekit.io/0gpwujpz1/No_image_available_T4YyeP1PdA.png';
  ...
  return (
    <ModalWrapper
      ...
      {Object.keys(current).length > 0 && (
        <Wrapper
          length={current.attributes.synopsis.length}
          style={{maxHeight: '100%'}}>
          <Header source={ImageSource}>
            ...
            <RankWrapper>
              {current.attributes.popularityRank && (
                <RankText>
                  #{current.attributes.popularityRank} Popularity Rank
                </RankText>
              )}
              {current.attributes.ratingRank && (
                <RankText>
                  #{current.attributes.ratingRank} Rating Rank
                </RankText>
              )}
            </RankWrapper>
          </Header>
          <TitleWrapper>
            ...
            {current.attributes.episodeCount && (
              <DurationText>
                episode: {current.attributes.episodeCount} |{' '}
                {current.attributes.episodeLength &&
                  current.attributes.episodeLength + ' min'}{' '}
              </DurationText>
            )}

            {current.attributes.chapterCount && (
              <DurationText>
                chapter: {current.attributes.chapterCount}{' '}
              </DurationText>
            )}
          </TitleWrapper>
          <SinopseWrapper>
